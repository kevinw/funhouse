...
(function() {
  var Camera, DRAW_DEBUG, KEY, LIGHT_TOPOLOGY, Level, RENDER_MIRRORS, USE_UNIFORM, mirrors;
  ...
  Level = (function() {
            ...
            function() {
              ...
              for (x = _k = _ref1 = mirrorRoomRect.x1, _ref2 = mirrorRoomRect.x2; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; x = _ref1 <= _ref2 ? ++_k : --_k) {
                if ((_this.cells[x + ',' + y1] == null) && (_this.cells[x + ',' + (y1 - 1)] == null)) {
                  _results.push(_this.setCell(x, y1, 'fourmirror'));
                } else {
              ...
              for (x = _k = _ref1 = mirrorRoomRect.x1, _ref2 = mirrorRoomRect.x2; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; x = _ref1 <= _ref2 ? ++_k : --_k) {
                if ((_this.cells[x + ',' + y2] == null) && (_this.cells[x + ',' + (y2 + 1)] == null)) {
                  _results.push(_this.setCell(x, y2, 'fourmirror'));
                } else {
                  _results.push(void 0);
                }
              }
              ...
              for (y = _k = _ref1 = mirrorRoomRect.y1, _ref2 = mirrorRoomRect.y2; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; y = _ref1 <= _ref2 ? ++_k : --_k) {
                if ((_this.cells[x1 + ',' + y] == null) && (_this.cells[(x1 - 1) + ',' + y] == null)) {
                  _results.push(_this.setCell(x1, y, 'fourmirror'));
                } else {
                  _results.push(void 0);
                }
              }
              ...
              for (y = _k = _ref1 = mirrorRoomRect.y1, _ref2 = mirrorRoomRect.y2; _ref1 <= _ref2 ? _k <= _ref2 : _k >= _ref2; y = _ref1 <= _ref2 ? ++_k : --_k) {
                if ((_this.cells[x2 + ',' + y] == null) && (_this.cells[(x2 + 1) + ',' + y] == null)) {
                  _results.push(_this.setCell(x2, y, 'fourmirror'));
                } else {
                  _results.push(void 0);
                }
              }
    ...
    Level.prototype.getNeighbors = function(x, y) {
      var dir, dx, dy, neighbors, _i, _len, _ref, _ref1;
      neighbors = [];
      _ref = ROT.DIRS['4'];
      for (dir = _i = 0, _len = _ref.length; _i < _len; dir = ++_i) {
        _ref1 = _ref[dir], dx = _ref1[0], dy = _ref1[1];
        neighbors.push(this.cells[(x + dx) + ',' + (y + dy)]);
      }
      return neighbors;
    };

    Level.prototype._paint = function() {
      var cell, character, desc, key, n, names, neighbors, x, y, _i, _len, _ref, _ref1, _results;
      names = {
        fourmirror: 'd'
      };
      _ref = this.cellsByName['fourmirror'];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        key = _ref[_i];
        _ref1 = COORDS(key), x = _ref1[0], y = _ref1[1];
        ...
        desc = ((function() {
          ...
          for (_j = 0, _len1 = neighbors.length; _j < _len1; _j++) {
            n = neighbors[_j];
            _results1.push(names[n != null ? n.name : void 0] || '_');
          }
          return _results1;
        })()).join('');
        if ((character = BoxDrawing.symbolLookups[desc]) != null) {
          _results.push(this.chars[key] = character);
        } else {
          _results.push(void 0);
        }
      }
    ...
    Level.prototype._placeEntities = function(mapInfo, opts) {
      var didDoorAt, doorCount, entranceRoom, entranceRoomRect, exitRoom, exitRoomRect, failsafe, i, iswall, otherRooms, r, room, roomInfo, rooms, roomsByDoorCount, x, y, _i, _j, _k, _l, _len, _len1, _m, _ref, _ref1, _ref10, _ref11, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
        _this = this;
      ...
      _ref11 = this.findFreeCell(), x = _ref11[0], y = _ref11[1];
    ...
    Level.prototype.addEntity = function(entity, x, y, opts) {
      var b, cancelled, entityList, key, otherEntity, _i, _j, _len, _len1, _ref, _ref1;
      if (opts == null) {
        opts = {};
      }
      ...
      if (!opts.skipBump) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (otherEntity !== entity) {
            cancelled |= entity.bump(otherEntity, opts) === BUMP_CANCEL_MOVE;
            cancelled |= otherEntity.bump(entity, opts) === BUMP_CANCEL_MOVE;
          }
        }
    ...
    Level.prototype.draw = function() {
      var Color, add, args, baseColor, bg, camRect, cell, cellKey, cellList, ch, character, dir, drawx, drawy, dx, dy, dynamicLight, e, entityBgColor, entityColor, entityList, ex, ey, finalColor, fog, found, gx, gy, intensity, key, light, lightData, maxheight, maxwidth, mirrorchar, mirrorx, mirrory, multiply, neighbor, neighborKey, opts, planes, rayX, rayXDelta, rayY, rayYDelta, screenKey, screenX, screenY, signx, signy, topEntity, watx, waty, x, xDelta, y, yDelta, _i, _j, _k, _l, _len, _len1, _len2, _m, _ref, _ref1, _ref10, _ref11, _ref12, _ref13, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
      this.display.clear();
      ...
      for (y = _i = _ref = camRect.y1, _ref1 = camRect.y2; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; y = _ref <= _ref1 ? ++_i : --_i) {
        for (x = _j = _ref2 = camRect.x1, _ref3 = camRect.x2; _ref2 <= _ref3 ? _j <= _ref3 : _j >= _ref3; x = _ref2 <= _ref3 ? ++_j : --_j) {
          ...
          _ref4 = COORDS(key), x = _ref4[0], y = _ref4[1];
          ...
          if (entityList != null ? entityList.length : void 0) {
            ...
            if ((entityColor = topEntity.color) != null) {
            ...
            character = this.chars[key] || cell.character;
            bg = this.bgs[key] || null;
          }
          if (bg) {
            bg = Color.toRGB(multiply(bg, light));
          }
          this.display.draw(screenX, screenY, character, Color.toRGB(finalColor), bg || null, opts);
        }
      }
      ...
      this.debug = {};
      if (RENDER_MIRRORS) {
        assert(this.mirrorSeers.length === 1);
        e = this.mirrorSeers[0];
        _ref5 = [e.getX(), e.getY()], ex = _ref5[0], ey = _ref5[1];
        cellList = this.cellsByName['fourmirror'] || [];
        for (_k = 0, _len = cellList.length; _k < _len; _k++) {
          key = cellList[_k];
          if (!this.visible[key]) {
            continue;
          }
          if (!(mirrorchar = this.chars[key])) {
            continue;
          }
          _ref6 = COORDS(key), mirrorx = _ref6[0], mirrory = _ref6[1];
          found = void 0;
          _ref7 = BoxDrawing.mirrorPlanes[mirrorchar] || [];
          for (_l = 0, _len1 = _ref7.length; _l < _len1; _l++) {
            dir = _ref7[_l];
            signx = Math.sign(ex - mirrorx);
            signy = Math.sign(ey - mirrory);
            gx = signx && signx === dir[0] ? signx : 0;
            gy = signy && signy === dir[1] ? signy : 0;
            if (gx || gy) {
              neighborKey = KEY(mirrorx + gx, mirrory + gy);
              neighbor = this.cells[neighborKey];
              if (neighbor && neighbor.lightPasses) {
                found = dir;
              }
              break;
            }
          ...
          while (true) {
            ...
            (_ref9 = this.display).draw.apply(_ref9, args);
            planes = {};
            _ref10 = BoxDrawing.mirrorPlanes[this.chars[cellKey]] || [];
            for (_m = 0, _len2 = _ref10.length; _m < _len2; _m++) {
              dir = _ref10[_m];
              planes[dir[0] + ',' + dir[1]] = true;
            }
            if ((rayXDelta < 0 && planes['-1,0']) || (rayXDelta > 0 && planes['1,0'])) {
              rayXDelta = -rayXDelta;
            }
            if ((rayYDelta < 0 && planes['0,-1']) || (rayYDelta > 0 && planes['0,1'])) {
              rayYDelta = -rayYDelta;
            }
          }
        }
        if (DRAW_DEBUG) {
          ...
          for (key in _ref11) {
            ch = _ref11[key];
            _ref12 = COORDS(key), mirrorx = _ref12[0], mirrory = _ref12[1];
            _ref13 = this.camera.getScreenCoords(mirrorx, mirrory), watx = _ref13[0], waty = _ref13[1];
            if ((ch = this.debug[key]) != null) {
              this.display.draw(watx, waty, ch);
            }
          }
        }
      }
